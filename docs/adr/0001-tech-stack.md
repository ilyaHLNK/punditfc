...
| **@nestjs/cli**                      | `nest build` and `nest start --watch` are the loop every Nest instruction assumes, and the CLI is the only supported host for the Swagger plugin, which derives the OpenAPI schema from types instead of from duplicated decorators | `tsc --watch` with Node's own `--watch` (the same dev loop with one dependency fewer, but no path to the Swagger plugin and a setup no reviewer recognises)          |
...
| **Prettier**                         | One formatter for the whole repository, run on staged files, so formatting never reaches a review                                                                                                                                   | Formatting rules inside ESLint (slower, and style findings bury real ones), no formatter at all (diff noise)                                                         |
| **ESLint**                           | Flat config with type-aware rules; the linter this project needs catches a forgotten `await`, not a missing semicolon                                                                                                               | Biome (one fast binary, but a thinner rule set, no type-aware depth and far weaker recognition in job postings), no linter                                           |
| **typescript-eslint**                | Hands ESLint the type checker: `no-floating-promises`, `no-misused-promises`, `switch-exhaustiveness-check` — the rule that guards the scoring engine's exhaustive switch                                                           | Untyped ESLint rules only: pattern matching cannot tell whether an expression produces a Promise                                                                     |
| **eslint-config-prettier**           | Switches off the ESLint rules Prettier owns, so the two tools cannot disagree                                                                                                                                                       | `eslint-plugin-prettier` (runs Prettier as a lint rule: slower, and formatting diffs drown the findings that matter)                                                 |
| **eslint-plugin-simple-import-sort** | Auto-fixable import order, so a diff shows the change instead of reshuffled imports                                                                                                                                                 | Ordering by hand (drifts immediately), `eslint-plugin-import` (needs a resolver configured for ESM + TypeScript for one cosmetic rule)                               |
